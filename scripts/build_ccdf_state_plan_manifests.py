...
IN_MANUAL_NOTE = (
    ...
    "Not extracted in this run: the plan family is not fully complete (25 jurisdictions blocked, 3 needs_review after the 2026-09-10 retry), "
...
RESOLUTIONS: dict[str, dict] = {
    ...
    "ak": {"status": "blocked_primary_source", "index": "https://aws.state.ak.us/OnlinePublicNotices/Notices/View.aspx?id=215530",
               "reason": "TCP connect to aws.state.ak.us:443 timed out (curl 75 s; curl_cffi chrome impersonation 45 s); retried 2026-09-10T18:36Z, same failure"},
    "as": {"status": "blocked_primary_source", "index": "https://www.dhss.as/index.html",
               "reason": "TLS: www.dhss.as presents a self-signed, expired certificate (CN=cmaster70); verification impossible, not disabled; retried 2026-09-10T18:36Z, same failure"},
    "az": {"status": "blocked_primary_source", "index": "https://des.az.gov/services/child-and-family/child-care/child-care-and-development-fund-state-plan",
               "reason": "HTTP 403 (cloudflare) for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    ...
    "co": {"status": "blocked_primary_source", "index": "https://cdec.colorado.gov/resources/state-plans",
               "reason": "HTTP 403 (CloudFront) for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    "ct": {"status": "blocked_primary_source", "index": "https://www.ctoec.org/ccdf/",
               "reason": "HTTP 403 (cloudflare) for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    ...
    "ga": {"status": "blocked_primary_source", "index": "http://www.decal.ga.gov/BftS/CCDFPlan.aspx",
               "reason": "connection reset by peer (curl 56) from www.decal.ga.gov for plain curl and curl_cffi; retried 2026-09-10T18:36Z, same failure"},
    ...
    "id": {"status": "blocked_primary_source", "index": "https://healthandwelfare.idaho.gov/providers/child-care-providers/child-care-resources",
               "reason": "index page reachable; the plan is served from publicdocuments.dhw.idaho.gov (Laserfiche WebLink DocView id=32013), TCP connect timed out; retried 2026-09-10T18:36Z, same failure"},
    ...
    "ks": {"status": "blocked_primary_source", "index": "https://www.dcf.ks.gov/services/ees/Pages/Child_Care/Child-care-and-early-education.aspx",
               "reason": "TCP connect to www.dcf.ks.gov:443 timed out (curl 75 s; curl_cffi 45 s); retried 2026-09-10T18:36Z, same failure"},
    "ky": {"status": "blocked_primary_source", "index": "https://www.chfs.ky.gov/agencies/dcbs/dcc/Pages/default.aspx",
               "reason": "HTTP 403 for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    "la": {"status": "blocked_primary_source", "index": "https://www.louisianabelieves.com/resources/library/early-childhood-policy-guidance",
               "reason": "HTTP 403 (cloudflare) for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    ...
    "md": {"status": "blocked_primary_source", "index": "https://earlychildhood.marylandpublicschools.org/about/ccdf-state-plan",
               "reason": "landing page 200; the linked plan page https://earlychildhood.marylandpublicschools.org/2025-2027-ccdf-plan returns HTTP 403 'Access denied'; retried 2026-09-10T18:36Z, same failure"},
    ...
    "mn": {"status": "blocked_primary_source", "index": "https://dcyf.mn.gov/child-care-and-development-fund",
               "reason": "mn.gov redirects plain requests to a validate.perfdrive.com (ShieldSquare) bot challenge instead of the page; retried 2026-09-10T18:36Z: plain request same failure; curl_cffi chrome impersonation now receives the mn.gov page, which is only a meta-refresh to the new Lead Agency page https://dcyf.mn.gov/child-care-and-development-fund (Department of Children, Youth, and Families); that host redirects plain requests to the Radware validate.perfdrive.com challenge and answers curl_cffi chrome/safari17_0 impersonation with a certificate chain that fails verification (curl 60 'self signed certificate in certificate chain'; openssl s_client sees a valid Sectigo EV chain), so the plan page was never served; verification not disabled"},
    ...
    "mo": {"status": "blocked_primary_source", "index": "https://dese.mo.gov/childhood/child-care-subsidy/child-care-dev-fund",
               "reason": "plan link https://dese.mo.gov/media/pdf/acf-118-ccdf-ffy-2025-2027-missouri returns an HTML Drupal antibot form (HTTP 200) instead of the PDF for requests and curl_cffi chrome120; retried 2026-09-10T18:36Z, same failure"},
    ...
    "ne": {"status": "blocked_primary_source", "index": "https://dhhs.ne.gov/Pages/Child-Care-and-Development-Fund-Plan.aspx",
               "reason": "TCP connect to dhhs.ne.gov:443 timed out (curl 75 s; curl_cffi 45 s); retried 2026-09-10T18:36Z, same failure"},
    ...
    "nj": {"status": "blocked_primary_source", "index": INDEX,
               "reason": "plan PDF https://www.childcarenj.gov/ChildCareNJ/media/media_library/CCDF_State_Plan_for_New_Jersey_FFY25-27.pdf returns HTTP 403 (cloudflare) for requests and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    ...
    "ny": {"status": "blocked_primary_source", "index": "https://ocfs.ny.gov/main/childcare/stateplan/",
               "reason": "ocfs.ny.gov returns an F5/Shape JavaScript bot-challenge page (TSPD) instead of the index; retried 2026-09-10T18:36Z, same failure"},
    ...
    "mp": {"status": "blocked_primary_source", "index": "https://www.childcare.gov.mp/",
               "reason": "HTTP 403 (nginx) for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    ...
    "or": {"status": "blocked_primary_source", "index": "https://www.oregon.gov/delc/about-us/pages/state-plans.aspx",
               "reason": "DNS resolution of www.oregon.gov failed from this network (curl 6) for plain curl and curl_cffi; retried 2026-09-10T18:36Z, same failure"},
    ...
    "ri": {"status": "blocked_primary_source", "index": "https://dhs.ri.gov/regulations/state-plans",
               "reason": "HTTP 403 (cloudflare) for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    "sc": {"status": "blocked_primary_source", "index": "https://www.scchildcare.org/resources/",
               "reason": "TCP connect to www.scchildcare.org:443 timed out (curl 75 s; curl_cffi 45 s); retried 2026-09-10T18:36Z, same failure"},
    ...
    "tn": {"status": "blocked_primary_source", "index": "https://www.tn.gov/humanservices/information-and-resources/tdhs-reports-and-information.html",
               "reason": "requests read timeout; HTTP 403 for curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    "tx": {"status": "blocked_primary_source", "index": "https://www.twc.texas.gov/programs/child-care/data-reports-plans",
               "reason": "HTTP 403 (CloudFront) for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    "ut": {"status": "blocked_primary_source", "index": "https://jobs.utah.gov/occ/plans.html",
               "reason": "HTTP 403 (awselb) for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
    "vt": {"status": "blocked_primary_source", "index": "https://dcf.vermont.gov/CDD/CCDF",
               "reason": "index page 200; plan PDF https://outside.vermont.gov/dept/DCF/Shared%20Documents/CDD/Reports/CCDF-Plans/CCDF-Plan-2025-2027-Approved.pdf returns HTTP 403 'The requested URL was rejected' (F5) for requests and curl_cffi chrome120; retried 2026-09-10T18:36Z, same failure"},
    ...
    "va": {"status": "blocked_primary_source", "index": "https://www.childcare.virginia.gov/reports-resources/administrative-program-manuals-reports-and-data/virginia-child-care-plan",
               "reason": "HTTP 403 (AkamaiGHost) for requests with browser UA and curl_cffi chrome120/safari17_0/edge101; retried 2026-09-10T18:36Z, same failure"},
